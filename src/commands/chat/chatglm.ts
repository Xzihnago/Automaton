...
try {
  ...
  chatglmHistorys = JSON.parse(
    await readFile("configs/chatglmHistorys.json", "utf-8"),
  ) as string[][];
} catch (error) {
  if ((error as NodeJS.ErrnoException).code === "ENOENT") {
    await writeFile(
      "configs/chatglmHistorys.json",
      JSON.stringify(chatglmHistorys),
    );
  } else {
...
const chatglm: TChatCommand = {
  ...
  callback: async (message) => {
    if (
      ["1050360446798020618", "869943337807908885"].includes(message.channelId)
    ) {
      ...
      try {
        const res = await axios.post<TChatGLMResponse>(
          "http://localhost:8000",
          {
            prompt: text,
            history: [
              ...chatglmHistoryBase,
              ...chatglmHistorys.slice(-chatglmHistorysLength),
            ],
          },
        );
