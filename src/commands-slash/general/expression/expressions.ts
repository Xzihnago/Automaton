...
const getExpressionArray = (expression: string) =>
  expression.match(/[\w.]+|\(-[\w.]+\)|[()!~^/%+-]|\*\*?|\|\|?|&&?|>=?|<=?|>>|<<|==|!=/g) ?? [];

const getExpressionString = (tokenArray: string[]) =>
  tokenArray.join(" ").replace(/\( /g, "(").replace(/ \)/g, ")").replace(/\s+/g, " ");
...
const convertNotation = (
  ...
  if (original === "infix") {
    tokenArray = target === "postfix" ? tokenArray : tokenArray.slice().reverse();
    const [startBracket, endBracket] = target === "postfix" ? ["(", ")"] : [")", "("];

    for (const token of tokenArray) {
      // If token is end bracket
      if (token === endBracket) {
        while (tokenStack.length && tokenStack[tokenStack.length - 1] !== startBracket) {
      ...
      // If token is operator
        ...
        while (operators[tokenStack[tokenStack.length - 1]] <= operators[token]) {
          targetArray.push(tokenStack.pop() ?? "");
        }
        tokenStack.push(token);
      }
  ...
  // Postfix => Infix | Prefix
    ...
    targetArray.push(tokenStack.pop() ?? "");
    if (target === "infix") targetArray[0] = targetArray[0].substring(targetArray[0].indexOf(" ") + 1);
  }
