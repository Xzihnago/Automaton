...
const getDuelMessage = (source: string, target: string) => {
  const skills = {
    ...
    special: [
      (source: string, target: string) => `<@${source}> 發動了 \`爆裂魔法\`\n<@${target}> 被燒成了灰燼`,
      (source: string, target: string) => `<@${source}> 使用了 \`液壓機\`\n<@${target}> 成功進入二次元`,
    ],
  };

  if (Math.random() < 0.1) {
    return skills.special[Math.floor(Math.random() * skills.special.length)](source, target);
  } else {
...
const duel: TChatCommand = {
  ...
  implement: async (message) => {
    ...
    const awaiting = [];
    for (const [id, _member] of targets) {
      switch (id) {
